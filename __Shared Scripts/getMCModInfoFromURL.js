async function my_function (tp, url) {
    ...
    try {
        ...
        if (parsedModName) {
            ...
            if (!result.success) {
                const response = await tp.obsidian.requestUrl({url: `https://api.curseforge.com/v1/mods/search?gameId=432&slug=${parsedModName}`, headers: {'x-api-key': '$2a$10$EkupduORZf5ZimE/NnJSmuGcAs2vjE7NhzTs5Hik7xw0enXVXBDE.'}});
